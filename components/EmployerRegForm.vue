...
class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-white z-10"
    ...
    <div v-if="currentStep === 1">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div
class="border p-4 rounded cursor-pointer hover:bg-indigo-50 text-center"
...
v-model="employer_name"
        ...
        <v-text-field
v-model="employer_shortDescription"
...
v-model="email"
      ...
      <v-text-field
v-model="password" :append-inner-icon="show_password ? 'mdi-eye' : 'mdi-eye-off'"
        :type="show_password ? 'text' : 'password'"
        :error-messages="(password_meta.touched || triedNext) && password_error ? [password_error] : []" label="Пароль"
        variant="outlined" density="compact" class="w-100 mb-3" @click:append-inner="show_password = !show_password" />
