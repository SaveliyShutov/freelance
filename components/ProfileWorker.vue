...
<template>
    <v-container class="px-0 px-sm-4">
        <div class="bg-indigo-600 border-2 text-center rounded-md border-indigo-600 mb-6 py-3">
            <p class="text-white font-bold">Профиль исполнителя</p>
        </div>
        ...
        <v-row v-if="userStore.user" class="flex-wrap">
            <v-col cols="12" sm="12" md="4" lg="4" class="order-1 md:order-none">
                <v-card class="pa-4 mb-4 shadow-lg border border-gray-100">
                    <div class="flex flex-col items-center">
                        ...
                        <div class="mt-3 text-center">
                            <p class="text-lg font-medium">{{ userStore.user.worker_name }}</p>
                ...
                <v-card class="pa-4 mb-4 shadow-lg border border-gray-100">
                    ...
                    <p v-if="!editingAbout" class="text-body-1">{{ userStore.user.worker_description }}</p>
                ...
                <v-card class="pa-4 mb-4 shadow-lg border border-gray-100">
                    ...
                    <div class="mb-4">
                        <div class="flex justify-between items-center">
                            ...
                            <button v-if="!editingFields.name" variant="plain"
                                class="!text-indigo-600 hover:!text-blue-800"
                                @click="startEdit('name')">Изменить</button>
                            <div v-else class="flex">
                                <button class="!text-red-400 hover:!text-red-600 mr-2"
                                    @click="cancelEdit('name')">Отмена</button>
                                <button class="!text-blue-400 hover:!text-blue-600"
                                    @click="saveEdit('name')">Сохранить</button>
                            </div>
                        </div>
                        <div v-if="editingFields.name">
                            <v-text-field :model-value="editData.name" @update:model-value="val => editData.name = val"
                                @keyup.enter="saveEdit('name')" color="indigo" variant="outlined" density="comfortable"
                    ...
                    <div class="mb-4">
                        <div class="flex justify-between items-center">
                            ...
                            <button v-if="!editingFields.email" variant="plain"
                                class="!text-indigo-600 hover:!text-blue-800"
                                @click="startEdit('email')">Изменить</button>
                            <div v-else class="flex">
                                <button class="!text-red-400 hover:!text-red-600 mr-2"
                                    @click="cancelEdit('email')">Отмена</button>
                                <button class="!text-blue-400 hover:!text-blue-600"
                                    @click="saveEdit('email')">Сохранить</button>
                            </div>
                        </div>
                        <div v-if="editingFields.email">
                            <v-text-field :model-value="editData.email"
                                @update:model-value="val => editData.email = val" @keyup.enter="saveEdit('email')"
                                color="indigo" variant="outlined" density="comfortable" class="mt-1" hide-details>
                            </v-text-field>
                        </div>
                        <div v-else class="py-2 mb-3">
                    ...
                    <div class="mb-4">
                        <div class="flex justify-between items-center">
                            ...
                            <button v-if="!editingFields.phone" variant="plain"
                                class="!text-indigo-600 hover:!text-blue-800"
                                @click="startEdit('phone')">Изменить</button>
                            <div v-else class="flex">
                                <button class="!text-red-400 hover:!text-red-600 mr-2"
                                    @click="cancelEdit('phone')">Отмена</button>
                                <button class="!text-blue-400 hover:!text-blue-600"
                                    @click="saveEdit('phone')">Сохранить</button>
                            </div>
                        </div>
                        <div v-if="editingFields.phone">
                            <v-text-field :model-value="editData.phone"
                                @update:model-value="val => editData.phone = val" @keyup.enter="saveEdit('phone')"
                                color="indigo" variant="outlined" density="comfortable" class="mt-1" hide-details>
                            </v-text-field>
                        </div>
                        <div v-else class="py-2 mb-3">
                    ...
                    <div>
                        <div class="flex justify-between items-center">
                            ...
                            <button v-if="!editingFields.location" variant="plain"
                                class="!text-indigo-600 hover:!text-blue-800"
                                @click="startEdit('location')">Изменить</button>
                            <div v-else class="flex">
                                <button class="!text-red-400 hover:!text-red-600 mr-2"
                                    @click="cancelEdit('location')">Отмена</button>
                                <button class="!text-blue-400 hover:!text-blue-600"
                                    @click="saveEdit('location')">Сохранить</button>
                            </div>
                        </div>
                        <div v-if="editingFields.location">
                            <v-text-field :model-value="editData.location"
                                @update:model-value="val => editData.location = val" @keyup.enter="saveEdit('location')"
                                color="indigo" variant="outlined" density="comfortable" class="mt-1" hide-details>
                            </v-text-field>
                        </div>
                        <div v-else class="py-2 mb-3">
            ...
            <v-col cols="12" sm="12" md="4" lg="4" class="order-3 md:order-none">
                <v-card class="pa-4 mb-4 shadow-lg border border-gray-100">
                    <v-card-title class="text-indigo-600 text-h6 font-weight-bold">Выполненные заказы</v-card-title>
                    <!-- <v-list>
                        <v-list-item v-for="work in userStore.user" :key="work.id" class="mb-2">
