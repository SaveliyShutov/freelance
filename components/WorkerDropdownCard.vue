      ...
      <v-menu offset-y transition="slide-y-transition">
        <template #activator="{ props }">
          <v-btn v-bind="props" variant="text" class="px-1 py-2 d-flex align-center gap-2">
            <v-avatar v-if="userStore.employer?.avatar" class="border" :image="userStore.employer?.avatar" />
            <v-avatar v-else class="border">{{ userStore.employer?.name[0] }}</v-avatar>
            <div class="text-left">
              <p class="text-sm">{{ userStore.employer?.name }}</p>
