    ...
    <v-app-bar color="white" elevation="0">
      <v-container class="px-4">
        <v-row class="flex align-center">
          <v-col class="hidden md:flex text-2xl cursor-pointer" cols="5" @click="router.push(`/landing`)">
          ...
          <v-col class="md:hidden flex-col cursor-pointer" md:cols="6" @click="router.push(`/landing`)">
...
class="hidden md:flex gap-1 lg:gap-2 xl:gap-3 justify-end items-center text-xs lg:text-sm xl:text-base"
          ...
          <v-col class="md:hidden flex gap-3 justify-end" cols="4">
            ...
            <v-app-bar-nav-icon class="md:hidden flex gap-6" @click="drawer = !drawer" />
          </v-col>
        </v-row>
      </v-container>
    </v-app-bar>
    ...
    <div v-if="drawer">
      <v-navigation-drawer v-model="drawer" location="right" temporary>
        <v-list class="text-gray-600">
          <v-list-item class="border-b pl-5 pb-4" @click="router.push(`/profile/${userStore.user._id}`)">
            <div class="text-xl font-medium">{{ userStore.user?.worker_name }} {{ userStore.user?.worker_surname }}
            </div>
...
v-else class="block px-4 py-2 text-gray-600 hover:text-indigo-600 text-decoration-none"
          ...
          <v-list-item class="items-center flex px-4 py-2 text-red-600" @click="logOut()">
